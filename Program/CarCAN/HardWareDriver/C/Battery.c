...
// 	ADC1->SMPR2|=7<<3;     //通道1  239.5周期,提高采样时间可以提高精确度	 
	...
	ADC1->SMPR2&=~0x07;  //通道0采样时间清空	  
 	ADC1->SMPR2|=0x07;     //通道0  239.5周期,提高采样时间可以提高精确度
...
float GetVoltage()
{
	u16 ADVal;
	float vol;
	ADVal=Get_Adc_Average(0,5);
	vol = 3.3 *  ADVal/4095.0 * (51 + 5.1)/5.1  + REG_B;
...
int BatteryCheck(void)
{
	...
	float vol;
	ADVal=Get_Adc_Average(0,5);
	vol = 3.3 * ADVal/4096.0 * (51 + 5.1)/5.1 + REG_B;
	if((vol<9.0) || (vol>18.0))
		return 0;
	else
		return 1;
