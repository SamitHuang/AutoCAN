...
u8 WriteBuffer(u8 FAR* DataAddress, u8 DataCount){
  ...
  //for RAM
  if (((u32)DataAddress + DataCount - 1) <= RAM_END)
  {
    for(i=0; i<DataCount; i++)
...
void protocol_init (void) {
  ...
  /*CLOCKPHASE_1EDGE*/
    ...
    SPI->CRCPR = (u8)0X07;
		...
		SPI->CR1 |= 0x40; 
	#endif /*SPI init end*/
	#ifdef USART_Protocol
	/* --------------  Initialize the USART peripheral ---------------- */
		
		
		/* Set Rx/Tx for USART uses*/
...
void Transmit(u8 Data){
	...
	//wait for Tx empty
	sr = USART->SR;
  while(!(sr & 0x80/*TxNE*/)) sr = USART->SR;
  //send data
   USART->DR = Data;
  //wait for transmission complete
  sr = USART->SR;
  while(!(sr & 0x40/*TxNE*/)) sr = USART->SR;
	#endif
}//Transmit
...
u8 Receive(u8* ReceivedData){
  ...
  //wait for Rx full
	sr = USART->SR;
  while(!(sr & 0x20 /*RXNE*/)) sr = USART->SR ;
  //check if overrun or parity error
